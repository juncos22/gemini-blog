...
const PostModel = {
  ...
  create: (postData: Omit<Post, 'id' | 'createdAt' | 'author'> & { authorId: number }): Post => {
    const db = readDB();
    const newPost = {
  ...
  update: (id: number, updatedData: Partial<Omit<Post, 'id' | 'createdAt' | 'author'> & { authorId?: number }>): Post | null => {
    const db = readDB();
